...
plot_ref <- ref[lifetime_dts$digest[, list(lifetime_main = main), keyby=user_id]][pwr_dts$digest[, list(pwr_main = main), keyby=user_id]][vMF_dts$digest[, list(peak_main = main), keyby=user_id]]
#thing[, list(user_id, mains=interaction(lifetime_main, pwr_main, peak_main))][,.N,by=mains]
ggplot(plot_ref[life > 365*24*60*60 & ul > 2 & ul < 20, list(y=log10(usage), x=log10(life), pwr_main, lifetime_main, peak_main)]) +facet_grid(pwr_main ~ lifetime_main) + aes(x=x,y=y, color=factor(peak_main)) + geom_point(alpha=0.3) + theme_bw()
```
\elandscape\newpage