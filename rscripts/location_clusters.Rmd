...
temporal_cluster <- data.table(location_id = peaks[measure == "usage"]$location_id, vMFcluster = predict(movMF(pos, 3, list(maxiter=10000, nruns=20)), pos), key="location_id")
...
ggplot(peaks[temporal_cluster]) + theme_bw() + aes(x = peak, fill=factor(vMFcluster)) + facet_grid(measure ~ .) + geom_bar(binwidth=1) + labs(fill="von Mises-Fisher\n3-Sphere\nCluster")
```
...
pwr_cluster <- tot_pwr[pwr_divisions][peak_freq_cat][,
  list(peak_freq, pwr_cat = ifelse(tot_pwr < low,"low",ifelse(tot_pwr < hi, "med","hi"))),
...
pwr_multi_clusters
reduced_pwr_clust <- pre_pwr_clust[, list(pwr_clust = ifelse((login == logout) || (login == usage), login, logout)), keyby=location_id]
```
...
agg.dt <- cbind(agg.dt, predict(lg10lm, agg.dt, interval="prediction", level=1/3))[,
  lifetime_cat := ifelse(log10_total_duration-log10_unique_users < lwr, "low",
                  ifelse(log10_total_duration-log10_unique_users < upr, "mid", "high"))
][reduced_pwr_clust][temporal_cluster]
p <- ggplot(agg.dt) + theme_bw() +
  aes(y = log10_total_duration-log10_unique_users, x = log10_lifetime, color = factor(vMFcluster), size = pwr_clust, alpha = pwr_clust) +
...
categorizer_help <- function(opts) function(SD) SD[,
  c(list(tot_visits = sum(lengths), mn_reps=mean(lengths), switches=.N),
    lapply(opts, function(v) SD[values == v, sum(lengths)])
  )
]
...
categorizer <- function(src.dt, cat, ref) {
  ...
  mx_expr <- parse(text=paste0("list(main=",cat,"[which.max(prop)],share=max(prop))"))
  rle_dt <- src.dt[, rle(eval(parse(text=cat))), keyby=user_id]
  ...
  opts <- sort(unique(rle_dt$values))
  names(opts) <- paste0("cat_", opts)
  fun <- categorizer_help(opts)
  ## cast data, maybe?
  digest <- rle_dt[, list(tot_visits = sum(lengths), mn_reps=mean(lengths), switches=.N), keyby=user_id][ref][prop_lab]
...
# lifetime_rle <- cluster_rle_src[,rle(lifetime_cat),keyby=user_id]
# lifetime_prop <- cluster_rle_src[,list(prop=sum(logout-login)), keyby=list(user_id, lifetime_cat)][ref][,list(prop=prop/usage), keyby=list(user_id, lifetime_cat)]
# lifetime_cat_dt <- lifetime_prop[, list(main=lifetime_cat[which.max(prop)], majority=max(prop)>0.5), by=user_id]
# 
# pwr_rle <- cluster_rle_src[,rle(pwr_clust),keyby=user_id]
# pwr_prop <- cluster_rle_src[,list(prop=sum(logout-login)), keyby=list(user_id, pwr_clust)][ref][,list(prop=prop/usage), keyby=list(user_id, pwr_clust)]
# pwr_prop_dt <- pwr_prop[, list(main=pwr_clust[which.max(prop)], majority=max(prop)>0.5), by=user_id]
# 
# 
# vMF_rle <- cluster_rle_src[,rle(vMFcluster), keyby=user_id]
# vMF_prop <- cluster_rle_src[,list(prop=sum(logout-login)), keyby=list(user_id, vMFcluster)][ref][,list(prop=prop/usage), keyby=list(user_id, vMFcluster)]
# vMF_prop_dt <- vMF_prop[, list(main=vMFcluster[which.max(prop)], majority=max(prop)>0.5), by=user_id]
# 
# 
# digest_lifetime_rle <- lifetime_rle[,list(
#     tot_visits = sum(lengths),
#     lows=sum((values=="low")*lengths),
#     mids=sum((values=="mid")*lengths),
#     highs=sum((values=="high")*lengths),
#     mn_reps=mean(lengths),
#     switches = .N
#   ), keyby=user_id][ref][lifetime_cat_dt]
# digest_pwr_rle <- pwr_rle[,list(
#     tot_visits = sum(lengths),
#     lows=sum((values=="low")*lengths),
#     mids=sum((values=="mid")*lengths),
#     highs=sum((values=="high")*lengths),
#     mn_reps=mean(lengths),
#     switches = .N
#   ), keyby=user_id][ref][pwr_prop_dt]
# digest_vMF_rle <- vMF_rle[,list(
#     tot_visits = sum(lengths),
#     As=sum((values==1)*lengths),
#     Bs=sum((values==2)*lengths),
#     Cs=sum((values==3)*lengths),
#     mn_reps=mean(lengths),
#     switches = .N
#   ), keyby=user_id][ref][vMF_prop_dt]

# path_by_cluster <- paths[, list(user_id, lengths, loc_cluster = interaction(lifetime_cat, pwr_clust, vMFcluster))]
```

```{r, echo=F}
#ggplot(lifetime_dts$digest) + theme_bw() + aes(x=log10(switches/usage), fill=main, alpha=majority) + geom_bar() + labs(title="By Lifetime Categories") + scale_alpha_manual(values=c(0.6, 1))
#ggplot(pwr_dts$digest) + theme_bw() + aes(x=log10(switches/usage), fill=main, alpha=majority) + geom_bar() + labs(title="By Frequency Power Categories") + scale_alpha_manual(values=c(0.6, 1))
#ggplot(vMF_dts$digest) + theme_bw() + aes(x=log10(switches/usage), fill=factor(main), alpha=majority) + geom_bar() + labs(title="By von Mises-Fisher Categories") + scale_alpha_manual(values=c(0.6, 1))
```