...
```{r, echo=FALSE}
# shifthrs_trans <- function(hrs) {
#   ihrs <- as.integer(hrs)
#   scales::trans_new(
#     paste0("shift",ihrs,"hrs"),
#     function(y) {
#       if (!is.numeric(y)) return(y)
#       (floor(y) - ihrs) %% 24L
#     },
#     function(yinv) {
#       if (!is.numeric(yinv)) return(yinv)
#       (floor(yinv) + ihrs) %% 24L
#     },
#   )
# }
...
## handle day splits:
## if login_day == logout_day
##  (floor(login_time/3600) <= hour) & (hour  <= floor(logout_time/3600))
## else - n.b., already precluded logout_time > login_time, since enforced < 24hr sessions
##  hour >= login | hour <= logout
