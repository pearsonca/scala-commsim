...
req.packages <- c("data.table", "ggplot2", "reshape2", "grid", "scales", "animation", "randomForest","lubridate")
...
1. consider subset - subset of records? subset of users?  subset of locations?  want predictions about location "class", so subset of locations?  later, when locations classified, subset of users.
2. using subset and some predictor measurements

```{r,echo=F}
## principle components analysis
# weekday_zeroes <- data.table(weekday=factor(c("Sun", "Mon", "Tues", "Wed","Thurs","Fri","Sat"), ordered = T), usage = 0, key = "weekday")
# predictors <- function(SD) with(SD, {
...
#   ), reduced_store))
# })
# unique_locs <- unique(censor.dt$location_id)
# slice <- sort(sample(unique_locs, size = length(unique_locs)/5, replace = F))
# pca.dt <- censor.dt[, predictors(.SD), by=location_id]
# centered <- pca.dt[,lapply(.SD[,-1,with=F],function(col) col-mean(col)),]
# scaled <- centered[,lapply(.SD, function(col) col/sd(col))]
# prediction.dt <- scaled[slice]
# validation.dt <- scaled[!slice]
# res <- prcomp(prediction.dt[, -1, with=F], scale. = F)
# res$var <- res$sdev^2
# res$pve <- res$var/sum(res$var)
# keep <- res$pve > 0.01 # 1/length(res$pve)
# orig.p <- par(mfrow=c(1,sum(keep)-1))
# for (x in 1:(sum(keep)-1)) biplot(res, choices=c(x,x+1), scale = 0)
# par(orig.p)
# with(res, {
#   plot(pve, xlab="Principal Component", ylab="Proportion of Variance Explained", type="b", log="y")
#   abline(h=1/(length(pve)))
#   # abline(h=tail(pve, 1)*2)
#   plot(cumsum(pve), xlab="Principal Component", ylab="Cumulative Proportion of Variance Explained", ylim=c(0,1), type="b")
#   abline(h=0.9)
#   plot(log10(1-cumsum(pve)), xlab="Principal Component", ylab="Remaining Variance", type="b")
#   abline(a=0, b=log10(1-pve[1]))
#   #plot(pve/(1-c(0,head(cumsum(pve),-1))), xlab="Principal Component", ylab="Proportion of Remaining Variance Explained", ylim=c(0,1), type="b")
# })
# par(orig.p)
# res
# res$x[,]
...
Looking at day-of-week, week-of-year patterns:

```{r, echo=F}
byweekday.dt <- censor.dt[,
  .N,
  keyby=list(location_id, weekday)
]
loctotals <- byweekday.dt[,list(total=sum(N)), keyby=location_id]
weekday.kmeans.src <- byweekday.dt[loctotals][,list(distribution=N/total), keyby=list(location_id, weekday)]
kmByWeekDayData <- kmParse2(weekday.kmeans.src, 10, 5, "location_id", "weekday")
ggplot(weekday.kmeans.src[kmByWeekDayData$clustering]) +
  theme_bw() + facet_grid(. ~ cluster, scales="free") +
  aes(x=factor(location_id), y=weekday, fill=log(distribution)) +
  geom_raster() +
  scale_x_discrete("location_id", labels="")

byweek.dt <- censor.dt[,
  .N,
  keyby=list(location_id, week)
]
week.kmeans.src <- byweek.dt[
  byweek.dt[,list(total=sum(N)), keyby=location_id]
][,
  list(distribution=N/total), keyby=list(location_id, week)
]
kmByWeekData <- kmParse2(week.kmeans.src, 10, 5, "location_id", "week")
ggplot(week.kmeans.src[kmByWeekData$clustering]) +
  theme_bw() + facet_grid(. ~ cluster, scales="free") +
  aes(x=factor(location_id), y=week, fill=log(distribution)) +
  geom_raster() +
  scale_x_discrete("location_id", labels="")

bymonth.dt <- censor.dt[,
  .N,
  keyby=list(location_id, month)
]
month.kmeans.src <- bymonth.dt[
  bymonth.dt[,list(total=sum(N)), keyby=location_id]
][,
  list(distribution=N/total), keyby=list(location_id, month)
]
kmBymonthData <- kmParse2(month.kmeans.src, 10, 5, "location_id", "month")
ggplot(month.kmeans.src[kmBymonthData$clustering]) +
  theme_bw() + facet_grid(. ~ cluster, scales="free") +
  aes(x=factor(location_id), y=month, fill=log(distribution)) +
  geom_raster() +
  scale_x_discrete("location_id", labels="")
