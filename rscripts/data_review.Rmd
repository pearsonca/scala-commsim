...
```{r, echo=FALSE}
ggplot(censor.dt[,list(t_min=min(login),t_max=max(logout)), by=location_id]) +
  theme_bw() +
  aes(x=location_id, ymin=t_min, ymax=t_max) +
  geom_linerange() +
...
```{r, echo=FALSE}
weekly_ave <- censor.dt[,(length(unique(location_id))-2)/(max(login) - min(login))*60*60*24*7]
weekly_count <- censor.dt[,list(creation_week=(min(login_day) %/% 7)), by=location_id][,list(count=.N),by=creation_week]
ggplot(weekly_count) + theme_bw() + aes(x=creation_week, y=count) +
...
```{r, echo=FALSE}
creations <- setkey(censor.dt[,list(creation_day=min(login_day)), by=location_id][,list(count=.N), by=creation_day][,list(creation_day = creation_day - min(creation_day), count)], "creation_day")
zeros <- data.table(creation_day = 0:max(creations$creation_day), count=0, key = "creation_day")
creations <- merge(creations, zeros, all=TRUE)[,list(count=max(count.x, count.y, na.rm=T)),keyby=creation_day]
ggplot(creations) + theme_bw() + aes(x=creation_day, y=count) +
...
Unlike creation, elimination of hotspot IDs seems to be much more variable.  So they are steadily created, but the lifetime after creation is variable.

```{r,echo=FALSE}
# dt <- censor.dt[,list(duration=(max(logout) - min(login))/60/60/24/7), by=location_id]
# h <- hist(dt$duration, 0:ceiling(max(dt$duration)), plot=F)
# ggplot() +
#   theme_bw() +
#   aes(x=duration/60/60/24/7) +
#   geom_bar(binwidth=1) + labs(x="weeks of hotspot life", y="hotspots with that duration")
```

The usage profile of hostspots also varies.  We can use this to categorize "types" of locations using a *k-means* approach.  First let's look at grouping by typical *login* times:
...
hr_shf <- (orig - shft) %% 24
hr_lab <- (orig + shft) %% 24
base <- censor.dt[,list(login_hour=floor(login_time/60/60)), by=location_id][,list(count=.N),keyby=list(location_id, login_hour)][,list(login_hour=hr_shf[login_hour+1],count=count/max(count)),keyby=location_id]
# by max vs by sum
byid <- acast(base, location_id ~ login_hour, value.var = "count", fill = 0)
kmplotter <- function(n) {
  km <- kmeans(byid, n, nstart = 5)
  ggplot(base[,list(location_id,login_hour,count,cluster=factor(km$cluster[location_id]))]) + theme_bw() +
    aes(x = factor(location_id), y = login_hour, fill = count) +
    facet_grid(. ~ cluster, scales = "free") + scale_x_discrete("location", labels="") +
    geom_raster() + scale_y_discrete(name="login hour", labels=hr_lab, breaks=0:23) +
...
We can also look at logout times:
...
hr_shf <- (orig - shft) %% 24
hr_lab <- (orig + shft) %% 24
base <- censor.dt[,list(logout_hour=floor(logout_time/60/60)), by=location_id][,list(count=.N),keyby=list(location_id, logout_hour)][,list(logout_hour=hr_shf[logout_hour+1],count=count/max(count)),keyby=location_id]
# by max vs by sum
byid <- acast(base, location_id ~ logout_hour, value.var = "count", fill = 0)
kmplotter <- function(n) {
  km <- kmeans(byid, n, nstart = 5)
  ggplot(base[,list(location_id,logout_hour,count,cluster=factor(km$cluster[location_id]))]) + theme_bw() +
    aes(x = factor(location_id), y = logout_hour, fill = count) +
    facet_grid(. ~ cluster, scales = "free") + scale_x_discrete("location", labels="") +
    geom_raster() + scale_y_discrete(name="logout hour", labels=hr_lab, breaks=0:23) +
    labs(fill="proportion max")
}
print(kmplotter(4))
print(kmplotter(5))
