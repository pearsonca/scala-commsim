...
req.packages <- c("data.table", "ggplot2", "reshape2", "grid", "scales")
...
duration_group.dt <- censor.dt[90 <= login_day & login_day < 180, 
...
login_group.dt <- censor.dt[90 <= login_day & login_day < 180, 
  list(count = .N), 
...
ggplot(cross_data.dt) + theme_bw() +
  ...
  geom_point()
# for 90 day window
#  for extant locations
#   - what percent of active days fall in each login category?
#   - what percent of logins fall in each login category?
```