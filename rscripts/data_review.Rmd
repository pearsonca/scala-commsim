...
```{r, echo=FALSE}
src.dt <- fread("~/Dropbox/montreal/merged.o", header = F, sep=" ", colClasses = c("integer","integer","integer","integer"))
...
1. sessions exceeding `r max_hours` hours
...
setkey(censor.dt, login, logout, user_id, location_id)
save(censor.dt, file = "~/Dropbox/montreal/censored.Rdata")
# censor.dt[,list(
#   login_day = login %/% (24*60*60),
#   logout_day = logout %/% (24*60*60),
#   login_time = login %% (24*60*60)/(60*60),
#   logout_time = logout %% (24*60*60)/(60*60)
# ) ]
```
