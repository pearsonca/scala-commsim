...
par( mfrow = c(length(memCounts), length(meetInterval)),
     oma = c(0.1, 0.1, 0.1, 0.1),
     mar = c(0.1, 0.1, 0.1, 0.1))

loc.colors.red <- list()
loc.colors.blu <- list()
reds <- c("lightred", "red", "darkred")
blues <- c("lightblue", "blue", "darkblue")
for (i in 1:length(locCounts)) {
  loc.colors.red[[ locCounts[i] ]] <- reds[i]
  loc.colors.blu[[ locCounts[i] ]] <- blues[i]
}

for (cnt in memCounts) {
  for (interval in meetInterval) {
    plot(NULL, NULL, ylim = c(0, 1), xlim = c(365, 1800))
    for (loc in locCounts) {
      ...
      points(res[1,,1], maxes[,1], col="red", pch=6)
