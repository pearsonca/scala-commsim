...
setwd("output/analyze-test/")

srcfiles <- list.files(pattern = "fgcomm.rds", full.names = T)

lapply(srcfiles, function(fname) {
  src <- readRDS(fname)
  ## want score by day
  ## score = when no members, FPR = 1, TPR = 0
  ## if some members, TPR = # members present - 1 / # members, FPR = sum (probability of choosing a particular community * size of comm - # coverts / size)
  mlt <- melt(src, na.rm=F, id.vars = c("day","negpop","count"))
  mlt[, covert_id := as.integer(gsub("[^0-9]","", variable))]
  mlt[grepl("size",variable), variable:="community_size" ]
  mlt[grepl("covert",variable), variable:="community_id" ]
  dcast(mlt, day+negpop+count ~ variable)
  mlt[variable=="community_id",.N,keyby=list(day, negpop, count, community_id=value)]
  mlt[variable=="community_size",.N,keyby=list(day, negpop, count, community_id=value)]
  inter <- mlt[,{
    res <- as.list(value)
    names(res) <- variable
    res
  },keyby=list(day,covert_id,negpop,count)][,
    list(covert_members=.N),
    keyby=list(day,negpop,count,community_id,community_size)
  ]
  fin <- melt(inter[,{
    not_found <- max(covert_members[is.na(community_id)],0,na.rm=T)
    sensitivity <- sum(0, covert_members^2/(count^2-count*not_found) * !is.na(community_id), na.rm = T)
    average_misid <- sum((covert_members/(count-not_found)) * community_size, na.rm = T)
    specificity <- 1 - (average_misid / negpop)
    list(sensitivity=sensitivity, specificity=specificity)
  },keyby=list(day,negpop,count)], id.vars = "day", measure.vars = c("sensitivity","specificity"))
  ggsave(gsub("rds","png",fname), ggplot(fin) + theme_bw() + aes(x=day, y=value, color=variable) + geom_step())
})
